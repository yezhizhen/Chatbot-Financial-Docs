...
def getAnswer(question,user,lang):
  ...
  if lang != 'en':
    print(lang)
    sys.stdout.flush()
    response.answer = translator.translate(result['answer'], src='en', dest=lang).text
...
def answer():
  ...
  return getAnswer(request.args.get('question'),request.args.get('user'),request.args.get('lang', 'en'))
