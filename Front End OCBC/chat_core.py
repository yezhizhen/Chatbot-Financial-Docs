...
def get_retriever(window_length_of_years, num_initial_docs, market):
    ...
    print(f"Getting retriever with window length: {window_length_of_years}, docs limit: {num_initial_docs}, in {market}")    
    vectorstore = get_vectorstore(market)
    return latest_n_year_retriever(vectorstore.as_retriever(search_kwargs={"k":num_initial_docs}), window_length_of_years)

@st.cache_resource
def get_qa_chain(market, window_length_of_years, num_initial_docs):
    print(f"Getting chain with window length: {window_length_of_years}, docs limit: {num_initial_docs}, in {market}")    
