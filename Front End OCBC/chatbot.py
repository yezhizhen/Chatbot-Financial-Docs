...
def body():
    ...
    with col2:
        with open(path.join(st.session_state.relative_dir_name,"stocks.json"),"r") as f:
            stocks = json.load(f)
            if market == "all":
    ...
    with col1:
        #window_length_of_years, num_initial_docs
        window_len = st.slider(":rainbow[Window size of years]", min_value=1, max_value=4, value=1, help='If set to N, then will look into "latest N years" among search results')
    with col2:
        doc_num= st.slider(":rainbow[Max limit of documents used]", min_value=1, max_value=8, value=4, 
